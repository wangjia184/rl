...
model = tf.keras.Sequential(
    [
        tf.keras.Input(type_spec=tf.RaggedTensorSpec(shape=[None, 7], dtype=tf.float32)),
        tf.keras.layers.Dense(256,activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.004)),
        #tf.keras.layers.LayerNormalization(),
        tf.keras.layers.Dense(512,activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.004)),
        #tf.keras.layers.LayerNormalization(),
...
#model.save_weights("/Users/jerry/Documents/RL/initial_weights/", overwrite=True)
model.load_weights("/Users/jerry/Documents/RL/initial_weights/")
...
class agent():
  ...
  def train(self, states, rewards, actions):
    ...
    total_grads = [tf.zeros_like(var) for var in self.model.trainable_variables]
    ...
    self.opt.apply_gradients(zip(total_grads, self.model.trainable_variables))
...
driver.get("file:///Users/jerry/Documents/RL/JS-Flappy-Bird-master/index.html")
...
for episode in range(100000):
    ...
    state = [  state_dic['to_gnd'] / 100.0, state_dic['to_roof'] / 10.0, state_dic['to_floor'] / 10.0, state_dic['to_start'] / 100.0, state_dic['to_end'] / 100.0, state_dic['speed'] / 10.0, state_dic['to_next_roof'] / 100.0 ]
    while running:
...
model.save_weights("/Users/jerry/Documents/RL/checkpoint/")
