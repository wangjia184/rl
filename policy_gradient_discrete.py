...
model.load_weights(str(current_dir) + "/initial_weights/policy_gradient_discrete/")
...
class agent():
    def __init__(self):
        ...
        self.opt = tf.keras.optimizers.legacy.Adam(learning_rate=0.00001)
        self.gamma = 0.8
...
model.save_weights(str(current_dir) + "/checkpoints/policy_gradient_discrete/", overwrite=True)
